...
build: ## Build Prometheus image
	...
	@echo "=========================================================";
	@cd jdk/${JDK_VERSION_MAYOR}; docker build -t ${LABEL}/prometheus:${PROMETHEUS_VERSION_FULL} .;
	
run: ## Run the Prometheus container
	@echo "Running Prometheus with default options";
	@docker run --name prometheus -p ${HOST_PORT}:9090 ${LABEL}/jenkins:${PROMETHEUS_VERSION_FULL}

debug: ## Enter Prometheus container terminal to do some debugging
	@docker run -ti -p ${HOST_PORT}:9090 ${LABEL}/prometheus:${PROMETHEUS_VERSION_FULL} bash
